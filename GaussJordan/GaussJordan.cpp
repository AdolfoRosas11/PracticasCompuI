...
template <typename matriz>
void Intercambiar_Filas_Colum(matriz & miMatriz);
...
int main()
{
    ...
    // Ordenamos la matriz
    Intercambiar_Filas_Colum(miMatriz);
...
{
    int variables = miMatriz.size();
    float pivote1 = miMatriz[0][0]; // almacenar el valor del primer elemento
    for (int j = 0; j <= variables; j++) {
        miMatriz[0][j] /= pivote1; // dividir cada elemento por el primer elemento
    }

    // Eliminar fila 2 y 3
    // fila 2
    float inv_add = -miMatriz[1][0];
    for (int j = 0; j <= variables; j++){
        miMatriz[1][j] += miMatriz[0][j] * inv_add;
    }
    //  fila 3
    float inv_add1 = -miMatriz[2][0];
    for (int j = 0; j <= variables; j++){
        miMatriz[2][j] += miMatriz[0][j] * inv_add1;
    }
    ...
    float pivote2 = miMatriz[1][1]; // **** del segundo elemento
    for (int j = 1; j <= variables; j++) {
        miMatriz[1][j] /= pivote2;
    }

    // Eliminar fila 1 y 3
    // fila 1
    float inv_add2 = -miMatriz[0][1];
    for (int j = 1; j <= variables; j++){
        miMatriz[0][j] += miMatriz[1][j] * inv_add2;
    }
    //  fila 3
    float inv_add3 = -miMatriz[2][1];
    for (int j = 1; j <= variables; j++){
        miMatriz[2][j] += miMatriz[1][j] * inv_add3;
    }
    ...
    // Hacer 1 la 3ra fila
    float pivote3 = miMatriz[2][2]; // **** del segundo elemento
    for (int j = 2; j <= variables; j++) {
        miMatriz[2][j] /= pivote3;
    }

    // Eliminar fila 1 y 2
    // fila 1
    float inv_add4 = -miMatriz[0][2];
    for (int j = 2; j <= variables; j++){
        miMatriz[0][j] += miMatriz[2][j] * inv_add4;
    }
    //  fila 2
    float inv_add5 = -miMatriz[1][2];
    for (int j = 2; j <= variables; j++){
        miMatriz[1][j] += miMatriz[2][j] * inv_add5;
    }
}
...
{
    int variables = miMatriz.size();
    int columnas = miMatriz.size();
    // Fila iniciando con 0
    for(int i = 0; i < 4 ; i++) {
        if (miMatriz[0][0] == 0) {
            if (miMatriz[1][0] == 0) {
